...
<div id="buddy">
    <div class="row content-area">
        <div class="small-12 medium-offset-3 medium-6 large-offset-4 large-4 column column">
        	...
        	<div class="page" id="register-page">
        		...
        		<?php if ( 'registration-disabled' == bp_get_current_signup_step() ) : ?>
        			...
        			do_action( 'bp_before_registration_disabled' ); ?>
        				...
        				<p><?php _e( 'User registration is currently not allowed.', 'zola' ); ?></p>
        		...
        		<?php if ( 'request-details' == bp_get_current_signup_step() ) : ?>
        			...
        			<p><?php _e( 'Registering for this site is easy. Just fill in the fields below, and we\'ll get a new account set up for you in no time.', 'zola' ); ?></p>
        			...
        			<div class="register-section" id="basic-details-section">
        				...
        				<h4><?php _e( 'Account Details', 'zola' ); ?></h4>

        				<label for="signup_username"><?php _e( 'Username', 'zola' ); ?> <?php _e( '(required)', 'zola' ); ?></label>
        				<?php
        				...
        				<label for="signup_email"><?php _e( 'Email Address', 'zola' ); ?> <?php _e( '(required)', 'zola' ); ?></label>
        				<?php
        				...
        				<label for="signup_password"><?php _e( 'Choose a Password', 'zola' ); ?> <?php _e( '(required)', 'zola' ); ?></label>
        				<?php
        				...
        				<label for="signup_password_confirm"><?php _e( 'Confirm Password', 'zola' ); ?> <?php _e( '(required)', 'zola' ); ?></label>
        				<?php
        			...
        			<?php if ( bp_is_active( 'xprofile' ) ) : ?>
        				...
        				<div class="register-section" id="profile-details-section">
        					...
        					<h4><?php _e( 'Profile Details', 'zola' ); ?></h4>
        						...
        						<div<?php bp_field_css_class( 'editfield' ); ?>>
        							...
        							if ( bp_current_user_can( 'bp_xprofile_change_field_visibility' ) ) : ?>
        								<p class="field-visibility-settings-toggle" id="field-visibility-settings-toggle-<?php bp_the_profile_field_id() ?>">
        									<?php
        									printf(
        										__( 'This field can be seen by: %s', 'zola' ),
        										'<span class="current-visibility-level">' . bp_get_the_profile_field_visibility_level_label() . '</span>'
        									);
        									?>
        									<a href="#" class="visibility-toggle-link"><?php _ex( 'Change', 'Change profile field visibility level', 'zola' ); ?></a>
        								</p>

        								<div class="field-visibility-settings" id="field-visibility-settings-<?php bp_the_profile_field_id() ?>">
        									<fieldset>
        										<legend><?php _e( 'Who can see this field?', 'zola' ) ?></legend>

        										<?php bp_profile_visibility_radio_buttons() ?>

        									</fieldset>
        									<a class="field-visibility-settings-close" href="#"><?php _e( 'Close', 'zola' ) ?></a>
        			...
        			<?php if ( bp_get_blog_signup_allowed() ) : ?>
        				...
        				<div class="register-section" id="blog-details-section">
        					...
        					<h4><?php _e( 'Blog Details', 'zola' ); ?></h4>

        					<p><label for="signup_with_blog"><input type="checkbox" name="signup_with_blog" id="signup_with_blog" value="1"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes, I\'d like to create a new site', 'zola' ); ?></label></p>

        					<div id="blog-details"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?>class="show"<?php endif; ?>>

        						<label for="signup_blog_url"><?php _e( 'Blog URL', 'zola' ); ?> <?php _e( '(required)', 'zola' ); ?></label>
        						<?php
        						...
        						<label for="signup_blog_title"><?php _e( 'Site Title', 'zola' ); ?> <?php _e( '(required)', 'zola' ); ?></label>
        						<?php
        						...
        						<span class="label"><?php _e( 'I would like my site to appear in search engines, and in public listings around this network.', 'zola' ); ?></span>
        						<?php
        						...
        						<label for="signup_blog_privacy_public"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_public" value="public"<?php if ( 'public' == bp_get_signup_blog_privacy_value() || !bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes', 'zola' ); ?></label>
        						<label for="signup_blog_privacy_private"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_private" value="private"<?php if ( 'private' == bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'No', 'zola' ); ?></label>
        			...
        			<div class="submit">
        				<input type="submit" name="signup_submit" id="signup_submit" value="<?php esc_attr_e( 'Complete Sign Up', 'zola' ); ?>" />
        			</div>
        		...
        		<?php if ( 'completed-confirmation' == bp_get_current_signup_step() ) : ?>
        			...
        			<?php if ( bp_registration_needs_activation() ) : ?>
        				<p><?php _e( 'You have successfully created your account! To begin using this site you will need to activate your account via the email we have just sent to your address.', 'zola' ); ?></p>
        			<?php else : ?>
        				<p><?php _e( 'You have successfully created your account! Please log in using the username and password you have just created.', 'zola' ); ?></p>
        			<?php endif; ?>
